...
class Event extends DataObject
{
    ...
    public function getCMSFields()
    {
        ...
        $cancelledField = CheckboxField::create('IsCancelled', 'This event is cancelled');
        if ($this->IsExpired()) {
            $cancelledField->setDisabled(true);
        }
        ...
        $fields->addFieldsToTab('Root.Main', [
            ...
            FieldGroup::create(
                'All Day',
                CheckboxField::create('IsAllDay', 'No specified start or end times')
            ),
            FieldGroup::create(
                'Recurring',
                CheckboxField::create('IsRecurring', 'This event occurs more than once'),
                RRuleField::create('RRule', '')
            ),
            FieldGroup::create(
                'Cancelled',
                $cancelledField,
    ...
    {
        $upcomingEvents = self::get()
            ->exclude('IsRecurring', true)
            ->filter('End:GreaterThanOrEqual', date('Y-m-d 00:00:00'));

        $upcoming = ArrayList::create();

        $upcoming->merge($upcomingEvents);

        $upcomingEventsRecurring = self::get()->filter('IsRecurring', true);

        foreach ($upcomingEventsRecurring as $event) {
            $recurringEvents = $event->getRecurringEvents();

            $upcoming->merge($event->getRecurringEvents());
        }
